    ...
    if (channelIndex === -1 && channelId !== helpDeskChannel) {
      ...
      if (isConnected(ws)) {
        ...
        if (isConnected(ws)) {
          ...
          send_to_slack(
            helpDeskChannel,
            `<!channel> We have a new chat in room: <#${channelId}>`
          );
        }
      }
    } else {
      ...
      switch (body.request) {
        ...
        case 'get-latest-message':
          try {
            ...
            res.statusCode = 500;
            res.end(JSON.stringify({ error: 'Error fetching latest message' }));
          }
          break;
        case 'addUser':
        ...
        case 'live-support-session':
          try {
            ...
            if (availableChannelIndex !== -1) {
              ...
              if (wss.clients.size > 0) {
                ...
                wss.clients.forEach((client) => {
                  if (client.readyState === WebSocket.OPEN) {
                    ...
                    console.log(
                      `Client connected to channel: ${availableChannel}`
                    );
                  }
                });
              }
              ...
              res.end(
                JSON.stringify({ status: 'Message sent and client connected' })
              );
            } else {
