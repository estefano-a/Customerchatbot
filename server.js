...
// Function to handle chat API requests
function handleChatApiRequests(req, res) {
  if (req.method === 'POST') {
    let body = '';
    req.on('data', (chunk) => {
      body += chunk.toString();
...
function handleSlackEvents(req, res) {
  let body = '';
  req.on('data', (chunk) => {
    body += chunk.toString();
  });
  req.on('end', async () => {
    try {
      ...
      // Handle Slack events (e.g., button clicks)
      if (event.type === 'event_callback' && event.event.type === 'message') {
        // Your logic for handling Slack messages
...
const server = http.createServer((req, res) => {
  if (req.url === '/api') {
    // Handle chat API requests on the /api path
    handleChatApiRequests(req, res);
  } else if (req.url === '/slack/events') {
    // Handle Slack events on the /slack/events path
    handleSlackEvents(req, res);
  } else {
...
// Upgrade HTTP requests to WebSocket for the /ws path
